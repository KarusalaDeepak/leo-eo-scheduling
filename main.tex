...
This paper extends our LEO reliability research from reliability estimation to scheduling decisions. Prior work studied risk-aware visibility-window reliability using survival analysis \cite{deepak_visibility} and disagreement-aware service path selection under structural shift in open LEO measurements \cite{deepak_disagreement}. Here, the focus is EO scheduling: which meta-tasks should be observed, which satellites should observe them, and which delivery opportunities should be reserved under congestion.

Most classical satellite task-scheduling work focuses on observation-window assignment, priority maximization, or visibility-aware association. In congestion-stressed LEO networks, this is insufficient. A satellite may be visible for observation but unable to downlink the produced data before the deadline. Similarly, choosing the earliest or highest-priority meta-task may overload scarce S2G windows and reduce completed EO products. The key distinction of this paper is full-chain service completion under congestion: observation feasibility, S2G capacity, one-hop ISL fallback, multi-satellite atomic-task splitting, and deadline reliability are evaluated together before a meta-task is accepted. This paper does not claim a globally optimal scheduler; it proposes a reproducible online heuristic for this stricter scheduling model.
...
This section reviews work most closely related to RACS: LEO reliability and measurements, EO task scheduling, congestion-aware space networking, and the specific gap addressed by full-chain EO completion under downlink stress.
...
This section defines the scheduling entities, reliability and congestion metrics, and optimization objective used to evaluate full-chain EO completion.
...
The dataset provides three window sets: observation windows $\mathcal{W}^{obs}$, S2G downlink windows $\mathcal{W}^{s2g}$, and ISL windows $\mathcal{W}^{isl}$. Fig.~\ref{fig:system} shows the full-chain scheduling flow from EO meta-task arrival to visible-satellite filtering, atomic-task assignment, S2G/ISL delivery, and final reliability--congestion scoring. An observation window indicates that satellite $s$ can observe atomic task $j$ during $[b,e]$. An S2G window indicates that satellite $s$ can downlink to ground station $g$ with bandwidth $B$ during $[b,e]$. The corresponding capacity is
\begin{equation}
...
RACS first orders meta-tasks using deadline tightness, priority, and volume. For each meta-task, it searches all visible satellites for each atomic observation. Unlike a single-satellite policy, RACS may assign different atomic observations of the same meta-task to different satellites. This assumption is operationally meaningful for independent image tiles, sensing points, or sub-areas that can be collected separately and fused at the ground segment. It is less appropriate for monolithic observations that require one continuous pass by the same satellite. RACS then attempts direct split downlink. If direct S2G delivery is infeasible, it checks one-hop ISL relay opportunities and downlinks through the relay satellite. Relay is limited to one hop to keep online search bounded and reproducible. This is not a claim that deeper relay chains are unimportant in operational networks; it is a scoped design choice for the benchmarked scheduling problem.
...
The score in equation (6) addresses a weakness of priority-only and deadline-only rules: they may select meta-tasks that are locally favorable but consume scarce delivery capacity. The reliability term rewards deadline and downlink-capacity margin. The congestion term penalizes plans that nearly exhaust a selected S2G window. The split-gain term encourages multiple-satellite use only when it increases feasibility. The relay penalty discourages unnecessary ISL use when direct downlink is available. Thus, RACS is a feasibility-first, weighted multi-objective heuristic for complete EO service delivery.
...
\end{equation}
This is suitable for online heuristic scheduling because the algorithm checks feasible windows directly and reserves resources greedily. The dominant factor in the evaluated instances is the observation-candidate search over satellites and atomic tasks, followed by S2G/ISL delivery checks. Algorithm~\ref{alg:racs} summarizes the complete RACS procedure: meta-tasks are ordered by deadline tightness, priority, and data volume; atomic observations are assigned using visible satellites; direct split downlink is attempted first; and one-hop relay is searched only when direct delivery is infeasible. RACS does not solve a global mixed-integer program, so it trades optimality guarantees for low implementation complexity and fast rescheduling.
...
This section describes the benchmark dataset, compared policies, evaluation metrics, stress scenarios, and statistical/runtime procedures used in the experiments.
