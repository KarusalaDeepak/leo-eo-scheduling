...
LEO satellite constellations increasingly support Earth observation (EO), remote sensing, disaster monitoring, and low-latency space information services. EO scheduling is difficult because an image capture is useful only if the generated data are also delivered to a ground station before the deadline. A feasible schedule must therefore jointly handle observation windows, generated volume, satellite-to-ground (S2G) downlink capacity, inter-satellite link (ISL) opportunities, priorities, and deadlines.

This paper extends our LEO reliability research from reliability estimation \cite{deepak_visibility} and path selection \cite{deepak_disagreement} to EO scheduling under congestion.

Most satellite task-scheduling work focuses on observation-window assignment, priority maximization, or visibility-aware association. In congestion-stressed LEO networks, this is insufficient because a satellite may observe a target but fail to downlink the data before the deadline. The key distinction of this paper is full-chain service completion under congestion: observation feasibility, S2G capacity, one-hop ISL fallback, multi-satellite atomic-task splitting, and deadline reliability are evaluated together before a meta-task is accepted. The goal is a reproducible online heuristic for this stricter scheduling model, not a globally optimal scheduler.

The main contributions of this paper are:
\begin{itemize}
\item We formulate reliability-aware, congestion-stressed EO scheduling with atomic observations, S2G capacity, ISL relay opportunities, deadlines, and priorities.
...
LEO networks experience fast topology changes, intermittent visibility, and limited contact duration. Prior work has studied topology dynamics, routing, and emulation platforms such as Hypatia and StarryNet \cite{bhattacherjee2018space,kassing2020hypatia,lai2023starrynet}. Measurement studies report short-term variability, outages, and location-dependent performance \cite{mohan2024starlink,zhao2024lens}. Our earlier work estimated usable service time and drop risk \cite{deepak_visibility} and used predictor disagreement under structural shift \cite{deepak_disagreement}; this paper applies that reliability-aware perspective to EO scheduling with explicit downlink congestion.

Satellite EO scheduling is a constrained optimization problem involving observation windows, priorities, deadlines, and resource conflicts \cite{wang2019survey,chen2020satellite}. Recent EO networking work studies downlink-latency reduction through semantic communication and distributed satellite edge processing \cite{hassan2024semantic,mercado2024vessel}. Downlink bottlenecks and ISL-assisted routing are also central to modern LEO systems \cite{kodheli2021survey,lu2024opensn}. Unlike work focused mainly on observation assignment, semantic reduction, processing placement, or visibility-only association, RACS requires a complete observation-and-delivery chain and treats S2G/ISL delivery as a scheduling constraint rather than a post-observation assumption.

\section{System Model and Problem Formulation}
This section defines the entities, metrics, and objective for full-chain EO completion.
...
The dataset provides observation windows $\mathcal{W}^{obs}$, S2G windows $\mathcal{W}^{s2g}$, and ISL windows $\mathcal{W}^{isl}$. Fig.~\ref{fig:system} shows the scheduling flow. An S2G window with bandwidth $B$ has capacity
\begin{equation}
...
RACS orders meta-tasks by deadline tightness, priority, and volume, then searches visible satellites for each atomic observation. Different atomic observations of the same meta-task may use different satellites, which is meaningful for independent image tiles, sensing points, or sub-areas. RACS attempts direct split downlink first and checks one-hop ISL relay only when direct S2G delivery is infeasible.
...
\end{split}
\end{equation}
where $H_i$ is urgency, $M_q$ is split-assignment gain, $N^{relay}_q$ is relay count, and $D_q$ is normalized delay. The weights prioritize completion reliability and downlink margin over small delay differences; they are not learned parameters or claimed to be globally optimal.
...
We compare earliest-deadline-first (EDF), highest-priority-first (HPF), shortest-task-first (STF), congestion-unaware, reliability-only, single-satellite reliability-congestion, and RACS. Metrics are completion rate, normalized priority reward, deadline miss rate, mean congestion ratio, mean reliability score, and Jain fairness \cite{jain1984fairness}.
...
This section reports base-scenario performance, stress-scenario behavior, ablation evidence, statistical confidence, runtime, fairness, and the main limitations of the study.

\subsection{Base Normal and Congested Scenarios}
Table~\ref{tab:base_results} shows average results by scenario group, and Fig.~\ref{fig:completion} visualizes the corresponding base-scenario completion rates. In normal instances, RACS increases completion rate from 0.900 for reliability-only scheduling to 0.958, while reducing deadline miss rate from 0.100 to 0.042. In congested instances, the margin is smaller but consistent: completion improves from 0.713 to 0.743, and mean congestion ratio decreases from 0.323 to 0.272. This indicates that RACS accepts more meta-tasks while using less congested delivery plans.

\begin{table}[t]
\caption{Average base-scenario results by policy group. Comp. is the completion rate, Reward is normalized completed priority, Miss is the deadline-miss rate, and Cong. is mean downlink congestion ratio.}
...
The benefit of RACS is clearer under controlled stress. Table~\ref{tab:stress_results} reports representative stress results, while Fig.~\ref{fig:stress_completion} summarizes completion-rate changes across the stress scenarios. Under combined stress, RACS improves completion rate from 0.626 to 0.806 compared with reliability-only scheduling and reduces deadline miss rate from 0.374 to 0.194. The gain comes from using feasible visible satellites, splitting downlink across multiple windows, and using relay fallback when direct delivery is insufficient.
...
Fig.~\ref{fig:score_sensitivity} reports a score-weight sensitivity analysis under combined stress. Perturbing the congestion, uncertainty, and relay penalties by 0.5x and 1.5x keeps completion rate in a narrow range from 0.785 to 0.791. This supports the interpretation that the main gain is driven by full-chain feasibility expansion rather than by a fragile single coefficient choice.
...
The base congested results show moderate gains because many meta-tasks remain feasible for several policies. The stress experiments show where RACS is most useful. When bandwidth is reduced, data volume increases, or deadlines tighten, single-satellite and single-window policies lose feasible plans quickly. RACS remains stronger because it can use multiple satellites, reserve multiple downlink windows, and use relay-assisted delivery.

\subsection{Limitations and Threats to Validity}
RACS is a greedy online heuristic, not a globally optimal mixed-integer optimization solver. The results should therefore be interpreted as evidence that reliability and congestion awareness are useful for lightweight online scheduling, not as proof of optimality. The score weights in (6) are heuristic; the sensitivity analysis in Fig.~\ref{fig:score_sensitivity} is useful but not exhaustive. The dataset is synthetic/benchmark-oriented rather than a complete operational constellation trace, so absolute completion rates should not be interpreted as operator-level performance. The congestion model, ISL availability, and S2G bandwidth assumptions may differ from deployed constellations.

\section{Code and Dataset Availability}
The implementation, experiment scripts, manuscript source, and generated paper figures are available at \url{https://github.com/KarusalaDeepak/leo-eo-scheduling}. The experiments use the IEEE DataPort dataset ``Congestion-stressed LEO satellite network dataset for Earth observation task scheduling and routing'' \cite{dataport_eo}. The dataset is not redistributed with the code repository; users should download it from IEEE DataPort and place the JSON instance files in the repository's documented dataset directory before running the experiments.

\section{Conclusion}
This paper presented RACS, a reliability-aware congestion-stressed EO task scheduler for benchmarked LEO satellite networks. RACS evaluates observation feasibility, delivery capacity, congestion, reliability margin, uncertainty, split downlink, and bounded relay fallback before accepting a meta-task. On benchmarked congestion-stressed LEO EO instances, RACS improves completion rate by up to 18 percentage points over reliability-only scheduling under combined stress while also reducing deadline misses and downlink congestion. The main contribution is to show that full-chain reliability and congestion awareness improve lightweight EO scheduling when deadlines and downlink capacity are jointly stressed.
